...
;Version Information
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "ProductName" "LC2Search"
  VIAddVersionKey /LANG=${LANG_ENGLISH} "Comments" "LC2Search Client"
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription" "LC2Search"
...
Name "LC2Search"
Caption "LC2Search Install (c)by http://www.letztechance.org"
Icon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"
OutFile "LC2Searchinstall.exe"
...
InstallDir "$PROGRAMFILES\LETZTECHANCE.ORG\LC2Search"
InstallDirRegKey HKLM "Software\LETZTECHANCE.ORG\LC2Search" "LC2Search"
...
;	CreateShortCut "$SMPROGRAMS\$R0\All users LC2Search.lnk" $INSTDIR\LC2Search.exe
;Abort
;!else
...
;MessageBox MB_YESNO "Wollen Sie dieses Programm wirklich installieren ?" IDYES
  ...
  WriteRegStr HKLM SOFTWARE\WWW.LETZTECHANCE.ORG\LC2Search "Install_Dir" "$INSTDIR"

  ; write uninstall strings
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2Search" "DisplayName" "LC2Search (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2Search" "UninstallString" '"$INSTDIR\uninst.exe"'
;---Extrahiere Dateien --------------------->
  SetOutPath $INSTDIR
...
SectionIn 1 4 3
  ...
  WriteRegStr HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Search "StrTest_INSTDIR" "$INSTDIR"
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Search "DwordTest_0xDEADBEEF" 0xdeadbeef
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Search "DwordTest_123456" 123456
  WriteRegDword HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Search "DwordTest_0123" 0123
  WriteRegBin HKLM SOFTWARE\LETZTECHANCE.ORG\LC2Search "BinTest_deadbeef01f00dbeef" "DEADBEEF01F00DBEEF"
;  StrCpy $8 "$SYSDIR\LC"
...
Section "CreateShortCuts"
 ...
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Search\LC2Config.lnk" $INSTDIR\LC2Config.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Search\Uninstall.lnk" $INSTDIR\uninst.exe
 CreateShortCut "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Search\Support.lnk" $INSTDIR\letztechance.org.url
  SectionIn 1 2 3
...
UninstallText "Uninstall LC2Search ?"
;UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\nsis1-uninstall.ico"
UninstallIcon "${NSISDIR}\Contrib\Graphics\Icons\lc.ico"

Section "Uninstall"

  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LC2Search"
  DeleteRegKey HKLM "SOFTWARE\WWW.LETZTECHANCE.ORG\LC2Search"

;  Delete "$SMPROGRAMS\WWW.LETZTECHANCE.ORG\LC2Search\reg\*.*"
...
;  File /a "exe\lc2sys.dll"
  ...
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Search\LC2Search.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Search\uninst.lnk"
  Delete "$SMPROGRAMS\$R0\LETZTECHANCE.ORG\LC2Search\*.*"
